...
const initialVariables = () => {
    localStorage.setItem("categoryadmin", 1);
...
const getData = (offset, limit) => {
    $.ajax({
        url: `http://localhost:9090/booktypes/findAll/${offset}/${limit}`,
        type: "GET",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', 'Bearer ' + getCookie("userToken"));
        }
...
const createPagination = (contentSize) => {
    setTimeout(() => {
        ...
        for (let i = 1; i <= pageSize; i++) {
            let activeClass = "";
            if (Number(localStorage.getItem("categoryadmin")) === i) {
                activeClass = "active";
            }
            let li = $("<li/>", {
                ...
                click: () => {
                    localStorage.setItem("categoryadmin", i);
                    $("#customer-data").html("");
                    getData((i - 1) * 10, 10);
                }
            });
...
$(function () {
    $("#updatecategory").click(function () {
        let idcategoryupdate = $("#idcategoryupdate").val();
        let namecategoryupdate = $("#namecategoryupdate").val();
        ...
        if (!idcategoryupdate) {
            ...
            var CategoryDto = {};
            CategoryDto.id=idcategoryupdate;
            CategoryDto.typeName = namecategoryupdate;

            $.ajax({
                type: "PUT",
                url: "http://localhost:9090/booktypes/update",
...
$(function () {
    $("#deletecategory").click(function () {
        let idcategorydelete = $("#idcategorydelete").val();
        ...
        if (!idcategorydelete) {
            ...
            // var CategoryDto = {};
            // CategoryDto.id=idcategorydelete;

            $.ajax({
                type: "DELETE",
                url: "http://localhost:9090/booktypes/delete/"+idcategorydelete,
                // data: JSON.stringify(CategoryDto),
                // contentType: "application/json; charset=utf-8",
                // dataType: "json",
