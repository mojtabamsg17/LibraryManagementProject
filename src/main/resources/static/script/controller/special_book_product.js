...
const initEvent = () => {
    $searchBox.on("keyup", (e) => {
        if (e.which === 13 || event.keyCode === 13) {
            $.ajax({
                url: "http://localhost:9090/books/findByBookType/specialize" + $("#fullsearch").val(),
                type: "GET",
...
const getData = (offset, limit) => {
    $.ajax({
        url: `http://localhost:9090/books/findByBookType/specialize/${offset}/${limit}`,
        type: "GET",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', 'Bearer ' + getCookie("userToken"));
        }
...
const createPagination = (contentSize) => {
    setTimeout(() => {
        ...
        for (let i = 1; i <= pageSize; i++) {
            let activeClass = "";
            if (Number(localStorage.getItem("specialbooks")) === i) {
                activeClass = "active";
            }
            let li = $("<li/>", {
                ...
                click: () => {
                    localStorage.setItem("speciallbooks", i);
                    $("#customer-data").html("");
                    getData((i - 1) * 10, 10);
                }
            });
...
const getTotalData = () => {
    $.ajax({
        url: "http://localhost:9090/books/findByBookTypeCount/specialize",
        type: "GET",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', 'Bearer ' + getCookie("userToken"));
        }
