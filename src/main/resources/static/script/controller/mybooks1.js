...
const initEvent = () => {
    $searchBox.on("keyup", (e) => {
        if (e.which === 13 || event.keyCode === 13) {
            $.ajax({
                url: "http://localhost:9090/users/myBooks" + $("#fullsearch").val(),
                type: "GET",
            ...
            <th>تاریخ پس دادن</th>
                ...
                <tbody id="customer-data">
                    ...
                    $.each(resp, function (index, bookLoan) {//resp.dataList, (i, v) => {
                        let tr = ` <tr>
        <th scope="row">${i + 1}</th>
        <td style="text-align: center">${bookLoan.id}</td>
        <td style="text-align: center">${bookLoan.book.title}</td>
        <td style="text-align: center">${bookLoan.balance}</td>
        <td style="text-align: center">${bookLoan.reservationDate} </td>
         <td style="text-align: center">${bookLoan.returnDate}</td>
...
const getData = (offset, limit) => {
    $.ajax({
        url: `http://localhost:9090/users/myBooks`,
        type: "GET",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', 'Bearer ' + getCookie("userToken"));
        }
            ...
            $.each(resp, function (index, v) {
                if (o == 0) {
                    ...
                    for (let i = 0; i < v.length; i++) {
                        let tr = ` <tr>
        <td style="text-align: center">${v[i].id}</td>
