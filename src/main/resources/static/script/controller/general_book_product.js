...
const initEvent = () => {
    $searchBox.on("keyup", (e) => {
        if (e.which === 13 || event.keyCode === 13) {
            $.ajax({
                url: "http://localhost:9090/users/searchBooksDynamicSearch/" + $("#fullsearch").val(),
                type: "GET",
...
const createPagination = (contentSize) => {
    setTimeout(() => {
        ...
        for (let i = 1; i <= pageSize; i++) {
            let activeClass = "";
            if (Number(localStorage.getItem("generalbooks")) === i) {
                activeClass = "active";
            }
            let li = $("<li/>", {
                ...
                click: () => {
                    localStorage.setItem("generalbooks", i);
                    $("#customer-data").html("");
                    getData((i - 1) * 10, 10);
                }
            });
...
const getTotalData = () => {
    $.ajax({
        url: "http://localhost:9090/books/findByBookTypeCount/general",
        type: "GET",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', 'Bearer ' + getCookie("userToken"));
        }
